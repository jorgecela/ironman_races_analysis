...
# Streamlit App for Visualizing Ironman Results with Parquet File
...
@st.cache_data
def load_data():
    data_path = "https://drive.google.com/uc?export=download&id=1HB9MeySUp7P28-zCE4taQgnvZisFLcFc"
...
x_axis_limits = {
    "IRONMAN": {
        ...
        "bike": (3.5, 9),       # in hours
        "run": (2, 8),        # in hours
        "finish": (7.5, 18)     # in hours
    },
    "IRONMAN 70.3": {
        "swim": (18, 75),      # in minutes
        "bike": (1.8, 4.5),       # in hours
        "run": (1, 4),        # in hours
        "finish": (3.5, 9)      # in hours
    }
}
...
limits = x_axis_limits.get(selected_race_type, None)
...
st.title(f"{selected_race_type}: Overall Statistics")
st.markdown(
    """
